...
if [[ ! -s scala-${SCALA_VERSION}.tgz ]]; then
        wget -O scala-${SCALA_VERSION}.tgz https://downloads.lightbend.com/scala/${SCALA_VERSION}/scala-${SCALA_VERSION}.tgz
fi
if [[ ! -s sbt-${SBT_VERSION}.tgz ]]; then
        wget -O sbt-${SBT_VERSION}.tgz https://github.com/sbt/sbt/releases/download/${SBT_LATEST_RELEASE_TAG}/sbt-${SBT_VERSION}.tgz
fi

sed "s/@SCALA_VERSION/${SCALA_VERSION}/g" Dockerfile.template | sed "s/@SBT_VERSION/${SBT_VERSION}/g" > Dockerfile

sudo podman build --tag scala:v${SCALA_VERSION} .
sudo podman build --tag scala:sbt-v${SBT_VERSION} .
